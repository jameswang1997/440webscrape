...
df.bio = df1 %>% mutate(gender = ifelse(gender %>% str_detect('Male'), 'M', 'F'),
  ...
  select(gender, d1, d2,d3, cup, height, weight, status, subscribe, video_view1)
...
df.bio1 %>% sapply(anyNA)
cor(df.bio1$weight,df.bio1$height)
fit1 = lm(score ~d1 + d2 + d3 + cup + height + weight, data = df.bio1)
