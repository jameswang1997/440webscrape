...
d = star.link.samp %>% sapply(parseVid)

df.vid = do.call(rbind, d) %>% as.data.frame()
df.vid %>% write.csv('df.vid.csv', row.names = F)
# d %>% str
```
...
```{r}
d1 = read_csv("star_bio.csv")
d1 %>% dim
d2 = read_csv("star_bio1.csv")
d3 = read_csv("star_bio2.csv")
...
# bind all dfs
```{r}
x = c("star_bio.csv","star_bio1.csv","star_bio2.csv","star_bio3.csv")
 allframes = lapply(x,function(x)read.csv(x))
 sapply(allframes,nrow)

 df.bind = do.call(rbind,allframes)
write.csv(df.bind, 'star_bio_bind.csv', row.names = F)
```
